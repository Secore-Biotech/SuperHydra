"""Unit tests for BinanceKlinesArchiveSpotFetcher.

Synthetic in-memory archives; no network. Mirrors test_klines_archive_fetcher.py
exactly, swapping the URL pattern and fetcher class.

Tests:
  - Construction (interval handling, cache dir default)
  - Window filtering
  - Multi-month enumeration (shared helper, but verified for spot path)
  ...
  - SPOT URL PREFIX: archive request hits /data/spot/, not /data/futures/um/
"""
...
def _make_kline_csv_row(
    *, open_time_ms: int, open_p: str, high: str, low: str, close: str,
    ...
    """Build one CSV row in Binance klines format (identical for spot and perp)."""
...
SPOT_URL_PREFIX = "https://data.binance.vision/data/spot/monthly/klines/"
...
class TestConstruction:
    ...
    def test_custom_interval(self, tmp_path):
        f = BinanceKlinesArchiveSpotFetcher(interval="1h", cache_dir=tmp_path)
        assert f.interval == "1h"

    def test_empty_interval_raises(self):
        with pytest.raises(ValueError, match="interval must be non-empty"):
            BinanceKlinesArchiveSpotFetcher(interval="")

    def test_default_cache_dir_includes_interval(self):
        f = BinanceKlinesArchiveSpotFetcher(interval="5m")
        assert "5m" in str(f.cache_dir)

    def test_default_cache_dir_is_spot(self):
        """Cache directory must be distinct from perp fetcher's directory."""
        f = BinanceKlinesArchiveSpotFetcher(interval="1d")
        assert "spot" in str(f.cache_dir)


class TestValidation:
    def test_empty_symbol_raises(self, tmp_path):
        f = BinanceKlinesArchiveSpotFetcher(cache_dir=tmp_path)
...
class TestURLPattern:
    """Spot fetcher MUST hit /data/spot/, not /data/futures/um/."""

    def test_hits_spot_url(self, tmp_path):
        rows = [
            _make_kline_csv_row(
        ...
        expected_url = f"{SPOT_URL_PREFIX}BTCUSDT/1d/BTCUSDT-1d-2026-03.zip"
        transport = _StubTransport({expected_url: archive})
        f = BinanceKlinesArchiveSpotFetcher(
        ...
        assert len(result) == 1
        # Verify the actual URL called
        assert len(transport.calls) == 1
        assert transport.calls[0] == expected_url
...
class TestWindowFiltering:
    def test_filters_to_window(self, tmp_path):
        ...
        archive = _make_archive_bytes(rows)
        url = f"{SPOT_URL_PREFIX}BTCUSDT/1d/BTCUSDT-1d-2026-03.zip"
        f = BinanceKlinesArchiveSpotFetcher(
            cache_dir=tmp_path,
...
class TestArchiveAbsent:
    def test_404_returns_empty(self, tmp_path):
        ...
        # Subsequent call should hit cached .notfound marker (no new HTTP)
        ...
        assert result2 == []
        # No new HTTP call on the second pass
        assert len(transport.calls) == first_call_count

    def test_404_url_is_spot(self, tmp_path):
        """Even on 404, the URL hit must be the spot URL pattern."""
