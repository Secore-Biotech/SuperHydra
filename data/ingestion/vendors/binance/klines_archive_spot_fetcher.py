"""Binance Vision archive klines (OHLCV bars) fetcher for SPOT instruments.

Companion to BinanceKlinesArchiveFetcher (which fetches USDⓈ-M PERP klines).
Same canonical output shape (BinanceKline), same monthly-archive convention,
same retry/throttle/cache discipline.

Use cases:
  - Hedged carry strategy simulation requiring paired perp + spot prices.
  - Spot reference price reconstruction (basis computation).
  - Cross-product analytics (perp/spot premium, funding-vs-basis decomposition).

Spot archive URL pattern:
  https://data.binance.vision/data/spot/monthly/klines/
      {symbol}/{interval}/{symbol}-{interval}-{YYYY}-{MM}.zip

Spot CSV schema is identical to USDⓈ-M futures klines:
  open_time, open, high, low, close, volume, close_time, quote_volume,
  count, taker_buy_volume, taker_buy_quote_volume, ignore

The canonical BinanceKline dataclass is venue-stamped "binance" without
distinguishing spot vs perp. The product is encoded in the symbol
("BTCUSDT" is canonical for both spot and USDⓈ-M perp on Binance; caller
must track which is which by context). This mirrors the existing perp
fetcher's convention exactly.

Cache location: artifacts/cache/binance_klines_spot_{interval}/.
"""
...
def _default_cache_dir_for(interval: str) -> Path:
    return Path(f"artifacts/cache/binance_klines_spot_{interval}")


@dataclass
class BinanceKlinesArchiveSpotFetcher:
    """Pulls historical SPOT klines from data.binance.vision monthly archives.

    Mirrors BinanceKlinesArchiveFetcher's behavior exactly, differing only
    in the archive URL prefix (/data/spot/ vs /data/futures/um/) and the
    ...
    def fetch_window(
        ...
        """Return all spot klines whose open_time is in [start, end).
    ...
    def _fetch_archive(self, symbol: str, year: int, month: int) -> bytes | None:
        ...
        url_path = ARCHIVE_PATH_TEMPLATE_SPOT.format(
...
# ─── Module-private helpers (same shape as perp fetcher) ──────────────────
